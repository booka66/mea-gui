...
class MainWindow(QMainWindow):
    ...
    def setup_file(self):
        with h5py.File(self.file_path, "r") as file:
            ...
            self.compressionLevel = file["Well_A1/WaveletBasedEncodedRaw"].attrs["CompressionLevel"]
            self.framesChunkLength = file["Well_A1/WaveletBasedEncodedRaw"].attrs["DataChunkLength"]
            self.coefsChunkLength = math.ceil(self.framesChunkLength / pow(2, self.compressionLevel)) * 2
            self.toc = file["TOC"][:]
    ...
    def setup_ui(self):
        ...
        self.graphWidget.setTitle(f"Frames {self.frame_indices[0]}-{self.frame_indices[-1]} - Accumulating Data")
        ...
        try:
            with h5py.File(self.file_path, "r") as file:
                for frame_index in self.frame_indices:
                    ...
                    coefs_end = self.wavelet_toc[frame_index+1] if frame_index+1 < len(self.wavelet_toc) else None
                    chunk_coefs = file["Well_A1/WaveletBasedEncodedRaw"][coefs_start:coefs_end]
                    self.chunk_data.append(np.array(self.reconstruct_chunk(chunk_coefs)))
            self.update_plot()
    ...
    def reconstruct_chunk(self, chunk_coefs):
        try:
            ...
            for i in range(4):  # Exactly 4 sub-frames
                start = i * self.coefsChunkLength * self.nChannels + self.chIdx * self.coefsChunkLength
