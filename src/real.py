    ...
    def process_next_chunk(self):
        ...
        if self.chunk_index < len(self.toc):
            with h5py.File(self.file_path, "r") as file:
                ...
                coefs_end = self.wavelet_toc[self.chunk_index+1] if self.chunk_index+1 < len(self.wavelet_toc) else None
...
def choose_file():
    root = tk.Tk()
    root.withdraw()
    file_path = filedialog.askopenfilename(title="Select BRW file", filetypes=[("BRW files", "*.brw")])
    if not file_path:
